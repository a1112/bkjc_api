...
class SqlServer_3d0(DataBaseInterFace):
    ...
    def getSteelBySequence(self, number, start_seq_no):
        return self.getSteelByNum(number, False, start_seq_no, desc=False, sequence_cursor=True)

    def getSteelByNum(self, number, defectOnly=False, startID=None, desc = True, *, sequence_cursor=False):
        """
        ...
        with SteelRecord.Session() as session:
            try:
                ...
                if startID:
                    cursor_column = SteelRecord.Steel.SequeceNo if sequence_cursor else SteelRecord.Steel.ID
                    que = que.filter(cursor_column > startID)

                if desc:
                    ord_item = (SteelRecord.Steel.SequeceNo if sequence_cursor else SteelRecord.Steel.ID).desc()
                else:
                    ord_item = (SteelRecord.Steel.SequeceNo if sequence_cursor else SteelRecord.Steel.ID).asc()
